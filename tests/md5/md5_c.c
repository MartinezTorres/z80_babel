...
static void md5Update(MD5Context *ctx, const uint8_t *input, size_t input_len);
 ...
 */
extern uint8_t *md5_c(MD5Context *ctx, uint8_t *input, size_t input_len) {
...
static void md5Init2();
static void md5Update2(const uint8_t *input, size_t input_len);
static void md5Finalize2();

static void md5Step2(uint32_t *buffer, const uint32_t *input);

static uint32_t rotateLeft2(uint32_t x, uint32_t n);
 ...
 * Functions that will return a pointer to the hash of the provided input
 */
extern uint8_t *md5_c_2(void *_, uint8_t *input, size_t input_len) {
	
	(void)(_);
	
	md5Init2();
	md5Update2(input, input_len);
	md5Finalize2();
	return ctx.digest;
}
 ...
 * and save the result in the buffer. Also updates the overall size.
 */
static void md5Update2(const uint8_t *input_buffer, size_t input_len){
...
static void md5Finalize2(){
	...
	// Fill in the padding andndo the changes to size that resulted from the update
	md5Update2(PADDING, padding_length);
	ctx.size -= (uint64_t)padding_length;
...
static void md5Step2(uint32_t *buffer, const uint32_t *input){
	...
	for(unsigned int i = 0; i < 64; ++i){
		...
		DD = CC;
		CC = BB;
		BB = BB + rotateLeft2(AA + E + K[i] + input[j], S[i]);
		AA = temp;
	}
